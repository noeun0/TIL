...
- 인스턴스가 생성된 이후 `인스턴스.변수명`으로 접근



#### 이름공간 탐색 순서

- 클래스를 정의하면, 클래스가 생성됨과 동시에 이름공간 생성
- 인스턴스를 만들게 되면, 인스턴스 객체가 생성되고 해당되는 이름공간이 생성
- 어트리뷰트가 변경될 시, 변경된 데이터를 인스턴스 객체 이름 공간에 저장
- 특정한 속성에 접근하게 되면 인스턴스 -> 클래스 순으로 탐색
...
- 부모 클래스의 속성 , 기능 등을 물려받는 것!
- 재사용성이 높아짐 



`isinstance(object, classinfo)` : object가 classinfo의 인스턴스 인가

`issubclass(class, classinfo) ` : class가 classinfo를 상속했는가 subclass인가
...
- 두개이상의 클래스를 상속받는 경우
- 중복된 속성이나 메소드는 상속 순서에 따라 결정된다
- 상속 받은 모든 요소 활용 가능

#### super()

- 자식 클래스에 메서드를 추가로 구현할 수 있다.
- 부모 클래스의 내용을 사용하고자 할 때, super를 사용할 수 있음
